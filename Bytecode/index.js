...
function ReadInstructions(reader, ConstantReferences, usedInstrs) {
  const Instructions = [];
  const InstLen = reader.gInt();
  for (let Idx = 0; Idx < InstLen; Idx++) {
    const [A, B, C, D] = reader.gAscii(4);
    const InstBin = `${bits(D)}${bits(C)}${bits(B)}${bits(A)}`;
    const Opco = parseInt(InstBin.substr(26, 6), 2);
    const Type = opdata.Opcode[Opco];
    const Mode = opdata.Opmode[Opco];
...
function ReadConstants(reader, ConstantReferences) {
  const Constants = [];
  const LConst = reader.gInt();
  for (let Idx = 0; Idx < LConst; Idx++) {
    const Type = reader.gBits8();
    let Cons = null;
    switch (Type) {
      case 1:
    ...
    const Refs = ConstantReferences[Idx];
    if (Refs) {
...
ChunkDecode = function (tree, reader, insts = null) {
  ...
  for (let i = 0; i < Chunk.Instr.length; i++) SetupReferences(Chunk, Chunk.Instr[i]);
...
function addOpcode(chunk, pos, name, opcode) {
  ...
  if (chunk.Tree.Chunk.UsedInstructions != null) chunk.Tree.Chunk.UsedInstructions[name] = true;
  ...
  if (chunk.Tree.Opmap != null) chunk.Tree.Opmap[name] = ranName;
  if (chunk.Tree.ROpmap != null) chunk.Tree.ROpmap[ranName] = `${name}`;
...
module.exports = {
  async DecodeBytestring(byteString, settings) {
    console.log('BRUH,', byteString);
    const reader = new ReaderClass(byteString);

    if (settings.Debug) console.log('Dumping headers');
    const Headers = {
    ...
    if (settings.Debug) console.log(`Setting gInt and gSizet, ${Headers.IntSize}, ${Headers.Sizet}`);

    if (Headers.IntSize === 4) reader.gInt = reader.gBits32;
    else if (Headers.IntSize === 8) reader.gInt = reader.gByte8;
    else return error('Integer size not supported');

    if (Headers.Sizet === 4) reader.gSizet = reader.gBits32;
    else if (Headers.Sizet === 8) reader.gSizet = reader.gByte8;
    else return error('Sizet size not supported');

    if (settings.Debug) console.log('Dumping top chunk');
    const Tree = {
      Headers, Opmap: null, ROpmap: null, AddOpcode: addOpcode,
    };
