function frexp(arg) {
  ...
  const encoder = {
    ...
    from_int: (x) => {
      ...
      for (i = 0; i < 4; i++) {
        const c = x % 256;
        v += encoder.from_string(String.fromCharCode(c));
        x = Math.floor(x / 256);
      }
      return v;
    },
    ...
    from_string: (s) => {
      ...
      for (let i = 0; i < s.length; i++) {
        const hex = s.substr(i, 1).charCodeAt().toString(16);
    ...
    dump_char: (y) => encoder.from_string(String.fromCharCode(y)),
  
    dump_headers: (headers) => encoder.from_string(headers.Signature)
    ...
    dump_constants: (consts) => {
      ...
      for (let Idx = 0; Idx < consts.length; Idx++) {
        const const_ = consts[Idx];
        const type = const_ == null ? 0
    ...
    dump_tree: (tree) => {
      let bytecode = '';
      const headers = encoder.dump_headers(tree.Headers);
      const chunk = encoder.dump_chunk(tree.Chunk);
