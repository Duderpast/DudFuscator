...
function generateString(length) {
  let result = '';
  const characters = '_oOxXuUwWiI0123456789';
  const charactersLength = characters.length;
  result += characters.charAt(Math.floor(Math.random() * characters.length - 10));
  length--;
  for (let i = 0; i < length; i++) {
...
function generateSuperOperators(chunk, settings) {
  ...
  function ProcessInstr(ip = 0) {
    ...
    while (ip < (chunk.Instr.length - 1)) {
      ...
      ip++;
      if (ip >= (chunk.Instr.length - 1)) break;
      if (virtual.Instructions.length >= 5) {
        ProcessInstr(ip);
        break;
      }
  ...
  while (instrPoint < (chunk.Instr.length - 1)) {
    const inst = chunk.Instr[instrPoint];
    if (inst !== null) {
...
function visitChunk(chunk, opmap, settings = {}, top = false) {
  ...
  const consts = chunk.Const.map((v) => v);
  const fakeConstants = [
  ...
  const l = chunk.Const.length + 3;
  consts.push('HERRTT FUSCATOR > ALL');
  for (let i = 0; i < l; i++) consts.push(fakeConstants[Math.floor(Math.random() * fakeConstants.length)]);

  const constIdx = shuffleArray(consts.map((v, i) => i));
  const newConst = [];
  ...
  for (let Idx = 0; Idx < chunk.Instr.length; Idx++) {
    ...
    if (inst !== null) {
      switch (inst.Name) {
        ...
        case 'MOD':
        case 'POW':
          if (inst['2'] >= 256) inst['2'] = constIdx[inst['2'] - 256] + 256;

          // Only RK(C)
        case 'GETTABLE':
        case 'SELF':
          if (inst['3'] >= 256) inst['3'] = constIdx[inst['3'] - 256] + 256;
          break;
      ...
      if (settings.MaximumSecurity && (inst.Name === 'LOADK' || inst.Name === 'GETGLOBAL' || inst.Name === 'SETGLOBAL') && typeof chunk.Const[inst['2']] === 'string') {
        ...
        if (typeof encedConsts[chunk.Const[inst['2']]] !== 'number') {
          ...
          const newCi = chunk.Const.length;
          encedConsts[chunk.Const[inst['2']]] = newCi;
          if (typeof chunk.Const[inst['2']] === 'string') chunk.Const[newCi] = xorStrArr(chunk.Const[inst['2']], chunk.Tree.XORPrimary);
          else chunk.Const[newCi] = chunk.Const[inst['2']];
          inst['2'] = newCi;
        } else {
        ...
        if (inst !== null && opmap[inst.Enum] !== undefined && opmap[inst.Enum] !== null) inst.Enum = opmap[inst.Enum];
      } else {
        // Randomize opcodes
        if (inst !== null && opmap[inst.Enum] !== undefined && opmap[inst.Enum] !== null) inst.Enum = opmap[inst.Enum];
      }
  ...
  if (top === true && settings.UseSuperops) generateSuperOperators(chunk, settings);
...
module.exports = {
  ModifyTree: (tree, settings) => {
    ...
    const [opmap, ropmap] = (settings.Debug !== true || settings.RenameInstructions === true) ? scrambleOpcodes() : grabShit();
    if (tree.OpQueue != undefined) {
      for (let Idx = 0; Idx < tree.OpQueue.length; Idx++) {
        const n = tree.OpQueue[Idx];
        let A;
        if (settings.Debug) A = n;
        else {
          do {
            A = `x${genChars(1)}${generateString(10)}`;
          } while (ropmap[A]);
